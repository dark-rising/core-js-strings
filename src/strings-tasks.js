...
function countVowels(str) {
  ...
  const vowelsCodes = vowels.map((vowel) => vowel.charCodeAt(0));
  let sum = 0;

  for (let i = 0; i < str.length; i += 1) {
    if (vowelsCodes.includes(str.charCodeAt(i))) {
      sum += 1;
    }
  }
  ...
  const encodedWords = words
    .map((word) => {
      ...
      const encodedChars = chars.map((char) => {
        const originalChar = input.indexOf(char);

        return originalChar === -1 ? char : output.at(originalChar);
      });
